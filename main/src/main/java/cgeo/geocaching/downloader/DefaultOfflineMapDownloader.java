...
/**
 * Downloads a small, low-detail world map on first use, so that the map isn't blank before the
 * user has downloaded any offline maps of their own. Only fires once, and only if no offline
 * map file is already present.
 */
public final class DefaultOfflineMapDownloader {
    ...
    /**
     * Triggers a background download of the default world map, but only if this has never been
     * attempted before and no offline map file is present yet.
     */
    ...
    private static synchronized void downloadIfNeeded() {
        ...
        try {
            ...
            try (InputStream in = Network.getResponseStream(Network.getRequest(REMOTE_URL));
                ...
                if (in == null || out == null) {
                    Log.i("DefaultOfflineMapDownloader: could not download default world map (no connection or storage error)");
                    return;
                }
                IOUtils.copy(in, out);
            }
